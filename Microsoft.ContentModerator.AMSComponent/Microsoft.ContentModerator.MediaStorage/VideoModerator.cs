...
namespace Microsoft.ContentModerator.MediaStorage
{
	...
	public class VideoModerator
	{
		...
		private IAsset CreateAsset(UploadVideoStreamRequest uploadVideoRequest)
		{
			_processedAssetName = uploadVideoRequest.VideoName;
			var assetName = Path.GetFileNameWithoutExtension(_processedAssetName);

			IAsset inputAsset = _mediaContext.Assets.Create(assetName, AssetCreationOptions.None);
			var assetFile = inputAsset.AssetFiles.Create(_processedAssetName);
			...
			using (Stream stream = new MemoryStream(uploadVideoRequest.VideoStream))
			{
				...
				var locator = _mediaContext.Locators.CreateLocator(LocatorType.OnDemandOrigin, inputAsset, policy);
		...
		private PublishedUrlDetails PublishAsset(IAsset asset)
		{
